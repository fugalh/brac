class MarkovPlugin < Plugin
  ...
  def chat(m, params)
    ...
    seed2 = params[:seed2]
    if seed2.nil?
      ary = %w{is was went did had}
      seed2 = ary[rand(ary.size)]
    end
    line = generate_string(seed1, seed2)
    m.reply line 
  end
...
plugin.map 'chat about :seed1', :action => "chat"
plugin.map 'chat :seed1 :seed2', :action => "chat"
plugin.map 'chat :seed1', :action => "chat"
