...
class UrlPlugin < Plugin
  ...
  def fetch(url, limit=10)
    ...
    return nil unless res.header['content-type'] == 'text/html'
    case res
  ...
  def listen(m)
    ...
    if m.message =~ /(f|ht)tps?:\/\//
      if m.message =~ /((f|ht)tps?:\/\/.*?)(?:\s+|$)/
        ...
        # give some info about the URL

        fetch(urlstr) =~ /<title>([^<]+)<\/title>/i and 
	  m.reply(REXML::Text.new("[#{$1.strip}]").value)
